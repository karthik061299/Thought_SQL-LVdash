...
SELECT 
    ...
    DATE_TRUNC('month', `session_start_date_time`) AS `month`,
    DATE_TRUNC('year', `session_start_date_time`) AS `year`,
...
GROUP BY `month`, `os_platform`
ORDER BY `month` DESC, `os_platform`;
