...
-- Updated with enhanced error handling and optimized queries
-- =====================================================

-- Base metric view for PaaS Tracking Card data with enhanced structure
...
SELECT 
    ...
    `is_clicked_aip_order_accordian_order_confirmed` AS `clicked_accordion_confirmed`,
    `is_clicked_aip_order_accordian_order_processing` AS `clicked_accordion_processing`,
    `is_clicked_aip_order_accordian_order_shipped` AS `clicked_accordion_shipped`,
    `is_clicked_aip_order_accord` AS `clicked_accordion_general`,
...
-- Calculated measures view matching ThoughtSpot formulas exactly
...
-- Funnel analysis view for conversion tracking
CREATE OR REPLACE VIEW `team_css_analytics_prod`.`hpx_analytics`.`paas_tracking_card_funnel_analysis` AS
SELECT 
    `month_date_dimension` AS `analysis_month`,
    `os_platform_dimension` AS `platform`,
    
    -- Funnel Stages
    SUM(`viewed_paas_tracking_card`) AS `stage_1_viewed_card`,
    SUM(`clicked_expand`) AS `stage_2_expanded_details`,
    SUM(`confirmed`) AS `stage_3_order_confirmed`,
    SUM(`processed`) AS `stage_4_order_processed`,
    SUM(`shipped`) AS `stage_5_order_shipped`,
    SUM(`delivered`) AS `stage_6_order_delivered`,
    SUM(`onboarded`) AS `stage_7_setup_completed`,
    ...
    CASE 
        ...
        THEN ROUND((SUM(`clicked_expand`) * 100.0 / SUM(`viewed_paas_tracking_card`)), 2)
        ELSE 0.0 
    END AS `view_to_expand_rate`,
    
    CASE 
        WHEN SUM(`clicked_expand`) > 0 
        THEN ROUND((SUM(`confirmed`) * 100.0 / SUM(`clicked_expand`)), 2)
        ELSE 0.0 
    END AS `expand_to_confirm_rate`,
    
    CASE 
        WHEN SUM(`delivered`) > 0 
        THEN ROUND((SUM(`onboarded`) * 100.0 / SUM(`delivered`)), 2)
        ELSE 0.0 
    END AS `delivery_to_onboard_rate`,
    
    -- Overall Funnel Efficiency
    CASE 
        WHEN SUM(`viewed_paas_tracking_card`) > 0 
        THEN ROUND((SUM(`onboarded`) * 100.0 / SUM(`viewed_paas_tracking_card`)), 2)
        ELSE 0.0 
    END AS `end_to_end_conversion_rate`
...
-- Enhanced with improved structure, error handling,
-- and exact formula matching from ThoughtSpot TML
-- =====================================================